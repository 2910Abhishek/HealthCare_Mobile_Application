...
//share button is there but not with selection mechanism
  ...
  Widget build(BuildContext context) {
    return Scaffold(
            ...
            IconButton(
              ...
              onPressed: () {
                if (_selectedFiles.isNotEmpty) {
                  _shareFiles(context, _selectedFiles);
                  _toggleSelectionMode();
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Please select files to share')),
                  );
                }
              },
            ),
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final file = widget.reports[index];
          return ListTile(
            leading: _isImageFile(file)
                ? Image.file(file, width: 50, height: 50, fit: BoxFit.cover)
                : _isPdfFile(file)
            ...
            tileColor: _selectedFiles.contains(file) ? Colors.blue.withOpacity(0.2) : null,
          );
        },
      ),
    );
